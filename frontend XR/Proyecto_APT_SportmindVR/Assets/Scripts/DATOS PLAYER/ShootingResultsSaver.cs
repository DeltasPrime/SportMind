...
public class ShootingResultsSaver : MonoBehaviour
{
    ...
    public void SaveAll()
    {
        ...
        if (!string.IsNullOrEmpty(postEmotion))
            data.shootingPostEmotion = postEmotion;

        // 2) Rendimiento (int)
        int? rend = GetSelectedInt(rendimientoToggles);
        if (rend.HasValue)
            data.shootingRendimiento = rend.Value;

        // 3) Ritmo (int)
        int? ritmo = GetSelectedInt(ritmoToggles);
        if (ritmo.HasValue)
            data.shootingRitmo = ritmo.Value;

        // 4) Confianza (int)
        int? conf = GetSelectedInt(confianzaToggles);
        if (conf.HasValue)
            data.shootingConfianza = conf.Value;
    }
